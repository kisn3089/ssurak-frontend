...
import { useIsMobile } from "@ssurak/ui/hooks/use-mobile";
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "fixed h-[calc(100svh-56px)] z-10 w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex",
            side === "left"
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7 transition-none", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
